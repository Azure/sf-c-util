...
HRESULT test_fabric_async_operation_EndTestOperation(TEST_FABRIC_ASYNC_OPERATION_HANDLE test_fabric_async_operation, IFabricAsyncOperationContext* context, int* operation_result)
{
    ...
    if (test_fabric_async_operation == NULL)
    {
        LogError("Invalid arguments: TEST_FABRIC_ASYNC_OPERATION_HANDLE test_fabric_async_operation=%p, IFabricAsyncOperationContext* context=%p, int* operation_result=%p",
            test_fabric_async_operation, context, operation_result);
        result = E_INVALIDARG;
    }
    else
    {
